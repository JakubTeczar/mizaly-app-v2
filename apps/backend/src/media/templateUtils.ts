...
export type RenderMode = "file" | "http";
...
export function buildFontsTag(mode: RenderMode, baseUrl?: string): string {
  if (mode === "http") {
...
// Keeps a text box short - full post captions can run to several paragraphs,
// which would either overflow a fixed-height box or force a tiny font. Cuts
// to the first `maxSentences` sentences, then to `maxChars` on a word
// boundary if that's still too long, appending "..." whenever anything was
// actually cut.
export function truncateDescription(text: string, maxSentences = 2, maxChars = 200): string {
